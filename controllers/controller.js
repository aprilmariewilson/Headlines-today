...
router.get('/scrape', function (req, res) {
	...
	request('http://bleacherreport.com/', function (error, response, html) {
		...
		$('.articleContent').each(function (i, element) {
			...
			result.title = $(element).children(".articleTitle").text();

			// Collect the Article Link 
			result.link = $(element).children(".articleTitle").attr("href");
			console.log(result);
			...
			if (result.title !== "") {
				...
				if (titlesArray.indexOf(result.title) == -1) {
					...
					// Push the saved item to our titlesArray 
				...
				// Log that scrape is working, just the content was missing parts
					...
					console.log('Redundant Site Content. Not Saved to DB.')
				}
